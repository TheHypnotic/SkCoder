...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    ...
    eval_sampler = SequentialSampler(eval_data)
    if args.data_num == -1:
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=(args.train_batch_size // 2),
                                     num_workers=16, pin_memory=True)
    else:
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=(args.train_batch_size // 2))
    ...
    output_fn = os.path.join(args.res_dir, "test_{}.jsonl".format(criteria))

    dev_accs, predictions = [], []
    with open(output_fn, 'w') as f:
        for pred_nl, gold in zip(pred_nls, eval_examples):
            dev_accs.append(pred_nl.strip() == gold.target.strip())
            ex = {'source': gold.source.strip(), 'prediction': pred_nl.strip(), 'target': gold.target.strip()}
            f.write(json.dumps(ex)+'\n')

    bleu = round(_bleu(output_fn), 2)
...
def main():
    ...
    # Print the filenames
    print("Train Filename:", args.train_filename)
    print("Dev Filename:", args.dev_filename)
    print("Test Filename:", args.test_filename)
    ...
    if args.do_train:
        ...
        output_model_file = os.path.join(last_output_dir, "ckpt.tar")
        if os.path.exists(output_model_file):
            ckpt = torch.load(output_model_file, map_location='cpu')
            if args.n_gpu > 1:
            ...
            best_ppl, best_bleu_em = ckpt['ppl'], ckpt['bleu_em']
            logger.info("load last checkpoint from {}".format(last_output_dir))
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.save_last_checkpoints:
                ...
                model_to_save = model.module if hasattr(model, 'module') else model
                output_model_file = os.path.join(last_output_dir, "ckpt.tar")
                torch.save({'model': model_to_save.state_dict(), 'optimizer': optimizer.state_dict(), 
                            'scheduler': scheduler.state_dict(), 'epoch': cur_epoch+1, 'ppl': best_ppl, 'bleu_em': best_bleu_em}, 
                            output_model_file)
                # torch.save(model_to_save.state_dict(), output_model_file)
                logger.info("Save the last model into %s", output_model_file)
